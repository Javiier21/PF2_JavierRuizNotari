async function obtenerCotizacionesDestacadas() {
    const monedas = ["bitcoin", "ethereum", "litecoin", "solana", "chainlink", "sandbox", "polkadot"];
    ...
    try {
      ...
      return Object.entries(data).map(([nombre, info]) => ({
        ...
        cambio: info.usd_24h_change ? `${info.usd_24h_change.toFixed(2)}%` : 'N/A',
        volumen: info.usd_24h_vol ? `$${info.usd_24h_vol.toFixed(2)}` : 'N/A',
        marketCap: info.usd_market_cap ? `$${info.usd_market_cap.toFixed(2)}` : 'N/A',
      }));
    } catch (error) {
...
async function generarCotizacionesDestacadas() {
    const carouselInner = document.querySelector("#cotizacionesCarousel .carousel-inner");
    ...
    try {
      ...
      cotizacionesGrupos.forEach((grupo, grupoIndex) => {
        ...
        item.className = grupoIndex === 0 ? "carousel-item active" : "carousel-item";
      ...
      const cotizaciones = document.querySelectorAll("#cotizacionesCarousel .cotizacion-destacada");
      cotizaciones.forEach((cotizacion, i) => {
        cotizacion.addEventListener("click", () => mostrarDetallesCotizacion(cotizacionesDestacadas[i]));
    });
  ...
  function mostrarDetallesCotizacion(cotizacion) {
    ...
    Swal.fire({
      title: `Detalles de ${cotizacion.nombre}`,
